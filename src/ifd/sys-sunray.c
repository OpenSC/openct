...
ifd_sysdep_usb_bulk(ifd_device_t *dev, int ep, void *buffer, size_t len, long timeout)
{
    ...
    ct_debug("ifd_sysdep_usb_bulk: endpoint=%d direction=%d", ep, direction);
    if(direction) {
	if((bytes_to_process=usb_bulk_read(devices[dev->fd],ep,buffer,len,timeout))<0) {
	...
	if((bytes_processed=usb_bulk_write(devices[dev->fd],ep,buffer,bytes_to_process,timeout))!=bytes_to_process) {
...
void
workaround_timeout(int sig)
{
    ifd_debug(6, "SunRay: signal_handler: sig=%d", sig);
    switch(sig) {
	case SIGALRM:
	    ifd_debug(6, "SunRay: workaround_timeout: sig=SIGALRM -> longjmp");
	    longjmp(env,sig);
	default:
	...
	long timeout)
{
    ...
    ifd_debug(6, "SunRay: ifd_sysdep_usb_capture: len=%d timeout=%d", len, timeout);
#ifdef USB_READ_INTERRUPT_TIMEOUT_WORKAROUND
    ...
    timeout=0;
#endif
    if((bytes_to_process=usb_interrupt_read(devices[dev->fd],cap->endpoint,buffer,len,timeout))<0) {
	ualarm(0,0);
	ifd_debug(6, "ifd_sysdep_usb_capture: usb_interrupt_read failed: %s", strerror(errno));
	ct_error("usb_bulk read failed: %s", strerror(errno));
	return IFD_ERROR_COMM_ERROR;
    }
#ifdef USB_READ_INTERRUPT_TIMEOUT_WORKAROUND
		...
		void *data, size_t len, long timeout)
{
    ...
    ct_debug("ifd_sysdep_usb_control: dev->fd=%d handle=0x%x", dev->fd, devices[dev->fd]);
    if ((rc = usb_control_msg(devices[dev->fd],requesttype,request,value,index,data,len,timeout)) < 0) {
 ...
 */
int
ifd_sysdep_usb_open(char *name, int flags)
{
    struct usb_bus	*bus;
    struct usb_device	*dev;

    ct_debug("ifd_sysdep_usb_open: name=%s flags=%d", name, flags);
	...
	for (dev = bus->devices; dev; dev = dev->next) {
		...
		bus->dirname,
		dev->filename);
	    ct_debug("ifd_sysdep_usb_open: check device=%s", device);
	    if(strcmp(device,name)==0) {
		int i=0;
		ct_debug("ifd_sysdep_usb_open: found match name=%s device=%s", name, device);
		for(i=0;i<128;i++) {
		    if(devices[i]==NULL) {
			devices[i]=usb_open(dev);
			ct_debug("ifd_sysdep_usb_open: usb_open index=%d handle=0x%x", i, devices[i]);
			return i;
		    }
		}
	    }
