 ...
 */
static const char *
ps_get_status_string(ps_response_status_code_t status_code)
{
  static const ps_response_status_code_map_entry_t
              ps_response_status_code_map[] = {
    { PS_SUCCESS,                           "Success"                 },
    { PS_SLOTERROR_PROCEDURE_BYTE_CONFLICT, "Procedure Byte Conflict" },
...
ps_if_transmission_receive(ifd_device_t *dev,
  ...
  if(SENDING != device_data->if_state && RECEIVING != device_data->if_state) {
  ...
  if(RECEIVING != device_data->if_state) {
  ...
  for(;;) {
    ...
    if(0 < chunk_len) {
      ...
      rc = ifd_device_recv(dev, chunk_start, chunk_len, timeout);
    
      if(IFD_SUCCESS > rc) {
        ct_debug("ps_if_transmission_receive: error: %i", rc);
        goto out;
      }
 ...
 */
static int
ps_receive_from_ifd(ifd_reader_t *reader,
                    unsigned char *rbuf,
                    size_t rlen)
{
...
out:
  ...
  return (IFD_SUCCESS <= rc) ? IFD_SUCCESS : rc;
}
...
ps_card_status(ifd_reader_t *reader, int slot, int *status)
{
  ...
  if(PS_CARD_UNKNOWN == card_status) {
    ...
    for(;;) {
      ...
      if(0 == memcmp(packet_buf,
        ...
        inserted = TRUE;
                            ...
                            sizeof(PS_CARD_REMOVED_NOTIFICATION))) {
        continue;
      }
      ...
      if(inserted) {
        ...
        if(PS_CARD_INSERTED != device_data->card_status) {
          status_tmp = IFD_CARD_STATUS_CHANGED;
        }
      } else {
    ...
    switch(new_icc_proto) {
      ...
      case IFD_PROTOCOL_T1:
        ...
        ifd_protocol_set_parameter(slot->proto, IFD_PROTOCOL_T1_IFSC,
                                  (atr_info.TA[2] !=- 1) ? atr_info.TA[2] : PS_DEFAULT_T1_IFSC);
        ifd_protocol_set_parameter(slot->proto, IFD_PROTOCOL_T1_IFSD, PS_MAX_T1_IFSD);
        break;
...
ps_open(ifd_reader_t *reader, const char *device_name)
{
  ...
  if(IFD_SUCCESS > rc) {
    ct_error("ps_open: ifd_device_set_parameters returned error %i", rc);
    goto out;
  }
  ...
  rc = ps_transceive_instruction(reader,
                                 PS_SET_OPTION,
                                 sbuf,
                                 1,
                                 NULL,
 ...
 */
void
ifd_pertosmart_ac1038_register(void)
{
